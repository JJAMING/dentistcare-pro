...
type ViewFilterMode = 'all' | 'daily' | 'monthly' | 'newMonthly';
...
const PatientList: React.FC<PatientListProps> = ({ patients, onRefresh }) => {
  ...
  useEffect(() => {
    if (location.state?.viewMode === 'newMonthly') {
      setViewMode('newMonthly');
  ...
  const handleSyncAll = async () => {
    if (!window.confirm('전체 환자의 덴트웹 예약 정보를 동기화하시겠습니까?\n환자 수에 따라 시간이 다소 소요될 수 있습니다.')) return;

    setIsSyncingAll(true);
    const currentPatients = [...patients];
    const patientsToSync = currentPatients.filter(p => p.chartNumber);
    setSyncProgress({ current: 0, total: patientsToSync.length });

    let updatedCount = 0;

    for (let i = 0; i < patientsToSync.length; i++) {
      const p = patientsToSync[i];
      setSyncProgress({ current: i + 1, total: patientsToSync.length });

      try {
        const result = await dentwebService.syncPatientFromDentweb(p.chartNumber);
        if (result.success) {
        ...
        console.error(`Error syncing patient ${p.name}:`, error);
      }
    ...
    alert(`전체 동기화 완료!\n총 ${patientsToSync.length}명 중 ${updatedCount}명의 정보가 업데이트 되었습니다.`);
  };
